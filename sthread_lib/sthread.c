...
/* Nova função: cria thread com prioridade explícita */
sthread_t sthread_create_p(sthread_start_func_t start_routine, void *arg, int priority) {
  sthread_t newth;
  IMPL_CHOOSE(newth = sthread_pthread_create(start_routine, arg),
              newth = sthread_user_create_p(start_routine, arg, priority));
  return newth;
}
...
/* Nova função: modifica o nice da thread activa */
int sthread_nice(int nice) {
  return IMPL_CHOOSE(-1, sthread_user_nice(nice));
}

/* Nova função: imprime estado das runqueues */
void sthread_dump(void) {
  IMPL_CHOOSE((void)0, sthread_user_dump());
}
...
sthread_mutex_t sthread_mutex_init() {
  ...
  IMPL_CHOOSE(lock = sthread_pthread_mutex_init(), lock = sthread_user_mutex_init());
  return lock;
}
