 ...
 * Uses default dynamic priority (7).
 */
sthread_t sthread_create(sthread_start_func_t start_routine, void *arg);

/* Create a new thread with explicit priority (0=highest, 14=lowest).
 * Priorities 0-4 are static; 5-14 are dynamic.
 */
...
/* Modifies the nice value of the calling thread (0..10).
 * Returns the priority the thread will have in the next epoch
 * if it exhausts its quantum.
 */
int sthread_nice(int nice);

/* Prints the state of the active runqueue, expired runqueue,
 * and blocked list.
 */
